...
const StainsAnalyticalReagentsIndicators = () => {
  const products =
    navBarData[2]?.sections[0]?.sections[3]?.sections[6]?.products;
  const path = navBarData[2]?.sections[0]?.sections[3]?.sections[6]?.path;
  ...
  return (
    <div className="w-full bg-grayColor rounded-lg px-5 md:px-10 py-8 flex flex-col items-start justify-start gap-8 ">
      <h2 className="font-bold text-3xl text-darkerBlue text-center w-full">
        Stains Analytical Reagents Indicators
      </h2>
...
export default StainsAnalyticalReagentsIndicators;
